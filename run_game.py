...
if __name__ == "__main__":
	...
	parser.add_argument('-p', '--threshold', default=0.5, type=float, help="[OPTIONAL] Evaluate moves with a play rate greater than this threshold (0.0=evaluates everything, 1.0=evaluates the most popular move), Example: 20%% will skip any opening that is played less than 20%% of the time")
	parser.add_argument('--total-threshold', default=0.9, type=float, help="[OPTIONAL] Stop evaluating when total opening coverage exceeds this amount (0.0=skips everything, 1.0=evaluates everything) Example: 0.95 will evaluate 95%% of the most popular openings")
